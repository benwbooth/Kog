...
| Partial | Kog deliberately uses one toolbar hamburger menu instead of Cog's macOS menu bar. Implemented file/URL and playlist opening, Save As, Save Selection As, tag editing, playlist-removal, View-window (including Equalizer), playback, preferences, and quit commands retain icons, shortcuts, and state-based enablement; Open Recent, queue/stop-after-selection, previous/next album, repeat modes, and shuffle modes remain |
