...
| Partial | Playlist table | Metadata rows, alternating backgrounds, Ctrl/Command and Shift multi-selection, keyboard range selection, double-click playback, live play marker, filtering, batch removal, external/tree drop-add, internal multi-row drag reordering with a themed insertion marker, a playlist context menu, and decoder-provided subsong expansion | Every Cog column, sorting, column chooser/order/width persistence, queue, ratings, inline editing, playlist persistence, and corpus-driven comparison of Cog's exact drag gestures |
