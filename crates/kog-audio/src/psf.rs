...
impl HelperHeader {
    fn read(reader: &mut impl Read) -> io::Result<Self> {
        ...
        for length in lengths {
            ...
            let text = kog_core::text_encoding::decode(&bytes).trim().to_owned();
...
fn validate_header(header: &HelperHeader, path: &Path) -> Result<(), String> {
    ...
    if header.format_version != u32::from(source_version)
        || !matches!(header.format_version, 1 | 2 | 0x23 | 0x24)
...
#[cfg(any(test, feature = "test-util"))]
pub fn test_snsf_bytes(offset: u32, data: &[u8], reserved: &[u8], tags: &str) -> Vec<u8> {
    ...
    let mut output = Vec::new();
    output.extend_from_slice(b"PSF\x23");
    output.extend_from_slice(&u32::try_from(reserved.len()).unwrap().to_le_bytes());
    output.extend_from_slice(&u32::try_from(compressed.len()).unwrap().to_le_bytes());
    output.extend_from_slice(&crc32(&compressed).to_le_bytes());
    output.extend_from_slice(reserved);
...
#[cfg(any(test, feature = "test-util"))]
pub fn test_snsf_rom() -> Vec<u8> {
    // Original 65C816 code uploads the original SPC700 program below through
    // the documented SNES IPL protocol. The SPC program installs a synthetic
    // BRR waveform and loops it on DSP voice zero; no Nintendo or game data is
    // embedded in this deterministic fixture.
    const CPU_PROGRAM: &[u8] = &[
        0x78, 0x18, 0xfb, 0xe2, 0x30, 0xad, 0x40, 0x21, 0xc9, 0xaa, 0xd0, 0xf9, 0xad, 0x41, 0x21,
        0xc9, 0xbb, 0xd0, 0xf2, 0xa9, 0x00, 0x8d, 0x42, 0x21, 0xa9, 0x02, 0x8d, 0x43, 0x21, 0xa9,
        0x01, 0x8d, 0x41, 0x21, 0xa9, 0xcc, 0x8d, 0x40, 0x21, 0xcd, 0x40, 0x21, 0xd0, 0xfb, 0xa2,
        0x00, 0xbd, 0x5d, 0x80, 0x8d, 0x41, 0x21, 0x8a, 0x8d, 0x40, 0x21, 0xcd, 0x40, 0x21, 0xd0,
        0xfb, 0xe8, 0xe0, 0x9b, 0xd0, 0xec, 0xa9, 0x00, 0x8d, 0x42, 0x21, 0xa9, 0x02, 0x8d, 0x43,
        0x21, 0x9c, 0x41, 0x21, 0x8a, 0x18, 0x69, 0x02, 0x8d, 0x40, 0x21, 0xcd, 0x40, 0x21, 0xd0,
        0xfb, 0x80, 0xfe,
    ];
    ...
    assert_eq!(CPU_PROGRAM.len(), 0x5d);
    assert_eq!(SPC_PROGRAM.len(), 0x9b);

    let mut rom = vec![0_u8; 0x8000];
    rom[..CPU_PROGRAM.len()].copy_from_slice(CPU_PROGRAM);
    rom[CPU_PROGRAM.len()..CPU_PROGRAM.len() + SPC_PROGRAM.len()].copy_from_slice(SPC_PROGRAM);
    rom[0x7fc0..0x7fd5].copy_from_slice(b"KOG SYNTHETIC SNSF   ");
    ...
    rom[0x7fda] = 0x33; // extended header marker
    rom[0x7fdb] = 0x00; // version
    rom[0x7fdc..0x7fde].copy_from_slice(&0xffff_u16.to_le_bytes());
    rom[0x7fde..0x7fe0].copy_from_slice(&0_u16.to_le_bytes());
    rom[0x7ffc..0x7ffe].copy_from_slice(&0x8000_u16.to_le_bytes());
    rom
}
